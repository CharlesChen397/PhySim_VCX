...
接下来，你只需要在终端中进入 `vcx-sim-master` 目录（下同），然后在命令行中输入 `xmake` 并执行，你就会看到 xmake 自动识别你的平台，下载所有依赖库并完成编译和链接；注意 Windows 下使用 gitbash 终端或者 Anaconda Powershell Prompt 可能无法正常编译，请使用 Windows PowerShell；注意这里可能遇到一些网络问题，可以参见FAQ中网络错误的部分。**若你使用 CMake 4.0 及以上（常见于较新的 Linux 发行版），请先阅读 FAQ 中「CMake < 3.5 has been removed」一条**，或使用项目根目录的 `./xmake.sh` 代替 `xmake` 命令。

继续执行 `xmake run lab0`（若使用包装脚本则为 `./xmake.sh run lab0`），如果一切顺利，你会看到一个界面，通过界面可以切换显示方框中的五个 Case。
...
- Q. 在 Linux 上首次执行 `xmake` / `xmake run lab0` 时，依赖包（如 glad、libccd）在 CMake 配置阶段报错「Compatibility with CMake < 3.5 has been removed」怎么办？

- A. 你本机的 **CMake 4.0 及以上**会拒绝过旧的 `cmake_minimum_required`，而 xmake 官方仓库里部分第三方包尚未更新。任选其一即可：
  - 在项目根目录使用包装脚本（已设置默认策略下限）：`./xmake.sh`（首次可执行 `chmod +x xmake.sh`），例如 `./xmake.sh run lab0`；
  - 或在当前终端先执行 `export CMAKE_POLICY_VERSION_MINIMUM=3.5`，再照常运行 `xmake`、`xmake run lab0`（说明见 [CMake 文档](https://cmake.org/cmake/help/latest/envvar/CMAKE_POLICY_VERSION_MINIMUM.html)）。
